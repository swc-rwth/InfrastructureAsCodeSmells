...
rule 'JUL060', 'Resources not following single responsibility principle' do
...
rule 'JUL061', 'Resources not following single responsibility principle - alternative' do
  tags %w{design multifaceted_abstraction}
  recipe do |ast, file|
    resarr = []
    find_resources(ast,type: 'execute').find_all do |cmd|
