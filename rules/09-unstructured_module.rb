rule 'JUL090', 'Metadata.rb is missing' do
...
rule 'JUL091', 'Template folder contains files missing the .erb file ending' do
  tags %w{design unstructured_module}
  cookbook do |filename|
    res_arr = []
...
rule 'JUL092', 'Attributes folder missing' do
    tags %w{design unstructured_module}
    cookbook do |filename|
...
rule 'JUL093', 'Recipes folder missing' do
  tags %w{design unstructured_module}
  cookbook do |filename|
...
rule 'JUL094', 'Attributes folder contains files with non .rb file ending' do
  tags %w{design unstructured_module}
  cookbook do |filename|
    res_arr = []
    attributes = File.join(filename,'attributes/')
...
rule 'JUL095', 'Recipes folder contains files with non .rb file ending' do
  tags %w{design unstructured_module}
  cookbook do |filename|
    res_arr = []
    recipes = File.join(filename,'recipes/')
